...
function onInit()
	...
	function storeNextChallenge($login, $i, $challengeName, $fileName)
	{
		foreach($this->listNextChallenges as $c)
		{
			if($c['login'] == $login)
			{
				$this->connection->chatSendServerMessage($this->connection->getPlayerInfo($login)->nickName.'$fffAlready juked a map:'.$c['name']);
				return;
			}
		}
		$this->listNextChallenges[] = array('id'=>$i, 'name'=>$challengeName, 'login'=>$login, 'filename'=>$fileName);
		$this->connection->chatSendServerMessage($this->connection->getPlayerInfo($login)->nickName.'$fff Juked the map:$w$fff'.$challengeName.'');
	}
	
	function onEndMap($rankings, $challenge, $wasWU, $matchContinuesOnNextChallenge, $restartChallenge)
	{
		if(count($this->listNextChallenges) > 0 && !$restartChallenge && !$wasWU)
		{
